...
class GameSever {
    ...
    broadcast(data, excludeSocket = null)
    {
        ...
        {
            if(client !== excludeSocket && client.readyState === WebSocket.OPEN)  //===는 비교전에 암시적인 형변환을 하지 않음(값이나 타입 중 하나라도 다르면 false 반환)
            {
                client.send(message);
                sentCount++;
            }
        ...
        {
            console.log(`브로드 캐스트 완료 ${data.type} (${sendCount}명에게 전송)`);
        }
    }
