...
class BattleServer {
    constructor(port){
        ...
        this.clients = new Set();                   //중복이 없는 자료구조 (JavaScripts)
    ...
    setupServerEvents()
    {
        this.wss.on('connection', (socket) => {
            ...
            this.waitingPlayers.set(playerId, {
                socket : socket,
                id : playerId,
                name : `Player_${playerId, substr(-4)}`,   //!!!!!이거 오류 날수도 있음 substr() : 정의되지 않은 함수
                hp : 100,
                maxHp : 100,
                inBattle : false,
                battleId : null
    ...
    tryMatchPlayers()
    {
        while(this.waitingPlayers.length >= 2)
        {
            ...
            this.startBattle(playerId, player2Id);
        }
    }
    ...
    startBattle(player1Id, player2Id)
    {
        ...
        this.sendToPlayer(player1Id, {
            ...battleStartMsg,           //JavaScript의 스프레드 문법 (배열이나 문자열을 개별 요소로 분해 하여 결합하는 문법)
            oppoent: player2.name,       //!!!!!!!!!이게 맞나?
            yourTurn : true,
            isPlayer1 : true
        });
    ...
    processBattleAction(battle, attackId, action)
    {
        const attacker = this.players.get(attackId);
        const defenderId = battle.player1 === attackId ? battle.player2 : battle.player1;
        ...
        if(defender.hp <= 0)
        {
            this.endBattle(battle, attackId);
            return;
        }
        ...
        this.sendToPlayer(battle.player1Id, {         //!!! player1 인지 아니면 player1Id 인지 
            ...nextTurnMsg,
    ...
    handleDisconnect(playerId)
    {
        ...
        if(player && player.inBattle)
        {
            const oppenetId = battle.player1 === playerId ? battle.player2 : battle.player1;

            this.sendToPlayer(oppenetId, {
                type : 'opponentDisconnected',
                message : '상대방이 연결을 종료 했습니다. 당신이 승리 했습니다.'
            });

            //상대방 상태 초기화
            const oppenet = this.players.get(oppenetId);
            if(oppenet)
            {
                oppenet.inBattle = false;
                oppenet.battleId = null;
            }
